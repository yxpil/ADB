...
const HOP_BY_HOP = new Set(['connection', 'keep-alive', 'proxy-authenticate', 'proxy-authorization',
  'te', 'trailer', 'transfer-encoding', 'upgrade', 'host', 'x-adb-target']);
...
function forward(req, res, url, cfg, store) {
  ...
  function dispatch() {
    let reqJson = null;
    const reqText = reqBuf.length ? reqBuf.toString('utf8') : '';
    ...
    upReq.on('error', (err) => {
      ...
      store({
        ...
        preview: previewOf(reqJson, reqText), tags: ['error'], tools: reqInfo.tools,
    ...
    function finalize(status, resHeaders, contentType, isSSE, failed, errMsg) {
      ...
      const tags = [...new Set([...reqCats, ...respInfo.categories,
        ...((failed || (status ?? 0) >= 400) ? ['error'] : [])])];
      ...
      store({
        ...
        api_key_hint: apiKeyHint(req.headers),
        preview: previewOf(reqJson, reqText),
